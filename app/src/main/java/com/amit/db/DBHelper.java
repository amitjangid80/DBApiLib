...
public class DBHelper
{
    ...
    /**
     * Constructor of the class you have to set the db name first before using this class.
     *
    ...
    /**
     * Created By AMIT JANGID 2018 Feb 01 - Thursday - 02:57 PM Execute Database Operations - method
     * name
     * <p>
     * This method is an universal method which will perform all the operations of database like -
     * create, delete, insert, update,
     * <p>
     * parameters to be passed in this method are as follows: the parameters below are numbered and
     * has to passed in the same sequence
     * <p>
     * parameter #1:
     *
     * @param tableName         - This will be the name of the table on which the operations has to
     *                          be performed - FOR EXAMPLE: tableName = "User"
     *                          <p>
     *                          parameter #2:
     * @param values            - This parameter is an object of LinkedHashMap - this parameter will
     *                          contain set of Key and Value to create table or insert data or
     *                          update we will have to pass this parameter with data - FOR EXAMPLE:
     *                          - values.put("Name", "'Amit'") - this for inserting data and
     *                          updating data - values.put("Name", "TEXT") - this for creating
     *                          table
     *                          <p>
     *                          parameter #3
     ...
     *                          <p>
     *                          ** c - for creating new table - while doing create operation the
     *                          user has to pass the data type also with the field of the table FOR
     *                          EXAMPLE: values.put("Name", "TEXT") - this for creating table
     *                          <p>
     *                          ** d - for deleting values from table - FOR DELETING SINGLE RECORD
     *                          OR ROW - the user has to pass true for the parameter "hasConditions"
     *                          because delete operation cannot be performed without where clause or
     *                          condition so the user has to pass data to "conditionalValues"
     *                          parameter - FOR EXAMPLE: hasConditions = true then
     *                          conditionalValues.put("ID", "'1'")
     *                          <p>
     *                          - FOR DELETING ALL THE RECORDS OR ROWS - the user has to pass null
     *                          of values, false for hasConditions and null of conditionalValues -
     *                          FOR EXAMPLE: values = null, hasConditions = false, conditionalValues
     *                          = null
     *                          <p>
     *                          ** dr - for dropping the table
     *                          <p>
     *                          ** i - for inserting values in the table - while doing insert
     *                          operation - the values that the user has to pass is of LinkedHashMap
     *                          object read the comments of values parameter for how to pass the
     *                          values - the user has to pass the field name and following with the
     *                          value of the field if the field is of type string or text, then the
     *                          value should be in single quotes ('') if the field is of type
     *                          integer, then the user can pass the value without the quotes or with
     *                          quotes - FOR EXAMPLE: values.put("firstName", "'Amit'"),
     *                          values.put("active", "1") or values.put("active", "'1'") - better
     *                          option this way
     *                          <p>
     *                          ** u - for updating values of table - while performing update
     *                          operation - pass values with field name of the table and following
     *                          with values - pass hasConditions value as true and also pass
     *                          conditionalValues with data - FOR EXAMPLE: hasConditions = true and
     *                          conditionalValue.put("ID", "1") OR conditionalValues("ID", "'1'") -
     *                          this option is better values.put("firstName", "'Amit'"),
     *                          values.put("email", "'anythiing@gmail.com'")
     *                          <p>
     *                          ***********************************************************************************************
     *                          <p>
     *                          parameter #4
     * @param hasConditions     - This parameter is used when the user has to perform Update or
     *                          Delete operations - pass true when doing UPDATE OR DELETE operations
     *                          and pass false if not doing UPDATE OR DELETE operations for delete
     *                          operation false can be passed, if entire data of the table is to be
     *                          removed - when passing this parameter with true, then you have to
     *                          pass conditionalValues parameter as well
     *                          <p>
     *                          parameter #5
     * @param conditionalValues - This parameter is used when the hasConditions parameter is set to
     *                          true: - If the hasConditions parameter is true then
     *                          conditionalValues should have an Object of LinkedHashMap - FOR
     *                          EXAMPLE: conditionalValues.put("ID", "'1'") - for updating data at
     *                          this "ID" - If the hasConditions parameter is false then
     *                          conditionalValues can be null
     *
    ...
    /**
     * 2018 Feb 01 - Thursday - 03:52 PM Execute Select Query
     * <p>
     ...
     * @param hasConditions     - if you want to use the where clause in select query then this
     *                          parameter should be set to true else this parameter can be false
     * @param conditionalValues - if the hasConditions is set to true then the user has to pass
     *                          conditionalValues else it can be null
     *                          <p>
     *                          the below lines are not in use so ignore it ** s - for selecting
     *                          values from table - pass * in values parameter when doing select
     *                          operations when you want to select every thing from the table no
     *                          matter condition is there or not - pass values parameters with the
     *                          name of the columns in the table when you want to select one or
     *                          multiple columns from the table no matter condition is there or not
     **/
    // endregion COMMENTS FOR executeQuery method
     ...
     *
     * @param tableName         - name of the table to perform select operation
     * @param values            - values to perform select query on Ex: "*" or "id, firstName"
     * @param hasConditions     - if you want to use the where clause in select query then this
     *                          parameter should be set to true else this parameter can be false
     * @param conditionalValues - if the hasConditions is set to true then the user has to pass
     *                          conditionalValues else it can be null
     * @param tClass            - Pass your Model class like this Ex: ModelClass.class this is
     *                          required for setting the values
     *
    ...
    /**
     * 2018 Feb 02 - Friday - 01:36 PM Get Record Count
     * <p>
     ...
     * <p>
     * ** @param hasConditions - if you want to get the count of a single record with some
     * conditions then set this as true else it will be false.
     * <p>
     * if this parameter is set to true then conditional values has to be provided else it won't
     * work.
     * <p>
     * ** @param conditionalValues - pass conditional values in this liked hash map it can be null
     * if hasConditions is set to false if hasConditions is set to true then this param has to be
     * passed.
     * <p>
     * *********************************************************************************************
     * <p>
    ...
    /**
     * 2018 September 07 - Friday - 05:39 PM is table exists method
     * <p>
     * this method will check if a table exists in database if yes is will not execute create table
     * query else it will execute
     *
    ...
    **/
    ...
    /**
     * 2018 August 13 - Monday - 12:34 PM get max field method
     * <p>
     * this method will get max value of the field in the table
     *
     * @param field     - primary key of the table to get the max value or an integer field of the
     *                  table to get the max value
    ...
    **/
    ...
    /**
     * 2018 August 20 - Monday - 04:01 PM execute query method
     * <p>
     * this method is used to execute a query this method will return true if the query is executed
     * successfully
     *
    ...
    public void insertDataWithTransaction(String tableName, int dbColumnCount)
    {
        ...
        for (int i = 0; i < dbColumnCount; i++)
        {
            for (DbData item : dbDataArrayList)
            {
                ...
                {
                    Log.e(TAG, "insertDataWithTransaction: for loop terminated because all the columns were found for table.");
                    break;
                }
