...
public class ToggleButton extends CardView
{
    @Retention(SOURCE)
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.ToggleButton,
                0, 0);

        if (a.hasValue(R.styleable.ToggleButton_tgMultiSelection)) {
            multipleSelection = a.getBoolean(R.styleable.ToggleButton_tgMultiSelection, false);
        }
        if (a.hasValue(R.styleable.ToggleButton_tgAllowDeselection)) {
            allowDeselection = a.getBoolean(R.styleable.ToggleButton_tgAllowDeselection, true);
        }
        if (a.hasValue(R.styleable.ToggleButton_toggleDividerColor)) {
            dividerColor = a.getColor(R.styleable.ToggleButton_toggleDividerColor, Color.GRAY);
        }
        if (a.hasValue(R.styleable.ToggleButton_customLayoutRes)) {
            layoutRes = a.getResourceId(R.styleable.ToggleButton_customLayoutRes, 0);
        }
        if (a.hasValue(R.styleable.ToggleButton_toggleModeWidth)) {
            @Mode
            int mode = a.getInt(R.styleable.ToggleButton_toggleModeWidth, MODE_WRAP);
            this.mode = mode;
        }
        selectedColor = a.getColor(R.styleable.ToggleButton_toggleSelectedColor,
                Utils.getThemeAttrColor(getContext(), R.attr.colorControlHighlight));
        //make sure this one is last
        if (a.hasValue(R.styleable.ToggleButton_toggleMenu)) {
            inflateMenu(a.getResourceId(R.styleable.ToggleButton_toggleMenu, 0));
        }
        a.recycle();
    }
